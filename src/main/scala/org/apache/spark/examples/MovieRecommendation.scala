  ...
  def main(args: Array[String]): Unit = {
    ...
    if (args.length != 5) {
      println("Usage: spark-submit --master yarn-client --class org.apache.spark.examples.MovieRater " +
              "movierecommendation-*-SNAPSHOT-jar-with-dependencies.jar ratingsCsvHdfsPath movieCsvHdfsPath newUserProfileCsvHdfsPath idOfUserToRecommendMovies modelLocation")
      sys.exit(1)
    }
    ...
    println("=================>>>> Re-Training and Saving Model... <<<<=====================")
    
    val model = ALS.train(trainingRDD, bestRank, bestNumIter, bestLambda)
